...
void Fit_Combine (int idx_file, bool doFlip, TString name_filein, TString name_fileout,
		  TString name_fileoutroot, FILE *file_result, int mode)

{
  ...
  //Characterize_Graph (graph_phase,     kBlack, "Phase",     bound_minFreq+0.0055, bound_maxFreq-0.0055);
  ...
  //cout << "-------------- coor_ampY: " << coor_ampY << endl;
  //cout << "-------------- nMidAmp: " << nMidAmp << endl;
  ...
  fitter.Config().ParSettings(2).SetLimits(scQ0QeAmp-abs(scQ0QeAmp*0.9), scQ0QeAmp+abs(scQ0QeAmp*3.));
  ...
  //fitter.Config().ParSettings(2).SetLimits(0.1, 5.); 
  fitter.Config().ParSettings(5).SetLimits(-10., 10.);
  ...
  fitPhase_test->SetParameter(2, par_Q0toQe*0.6);
  ...
  */
  ...
  // * Print plot name_fileoutto file
  ...
  */
  ...
  TString date_inf = ((TObjString*)arr_fname_date_time->At(4)) -> String();
  TString time_inf = ((TObjString*)arr_fname_date_time->At(5)) -> String();
  ...
  if ( chi2/ndf < 0.7)
    {
      fprintf (file_result, "%s.%s.%s %s:%s:%s %.6f %.0f %.0f %.6f %.5f %.1f %.1f %.6f\n",
	       yy.Data(), mm.Data(), dd.Data(), hou.Data(), min.Data(), sec.Data(),
	       par_omega, par_Q0, par_Qe, RM_posi, chi2/ndf, err_Q0, err_Qe, err_omega);
    }
...
// + Main function
//================
void fitting_modemap (bool doFlip, TString dir, TString ifile_run = "all", int mode = 1)
{
  // + Start the job
  //----------------
  printf (" * Job starts!\n\n\n");
