...
void CreateRootFile_Power_Temp(TString indir, TString sel_file, TString outdir, TString chan) {

	float time_start_cali = 182100;
	//TString fname_T7 = Form("../data/CD102/temperature/%s_T_211012_13.log", chan.Data());
	TString fname_T7 = Form("../data/CD102/temperature/%s", chan.Data());
	...
	while (fin_T7 >> dr_info) {
		...
		//if (start_time.Atof() < 221042) continue;  // this is for Oct-22 Calibration
		if (start_time.Atof() < time_start_cali) continue;  // this is for Oct-12 Calibration
		...
		if (temp_ < 0.6) continue;   // for Oct-12 calibration
	...
	for (unsigned int i = 1; i < vec_temp.size(); i++) {
		...
		if ((diff > 0.002 || diff < -0.002) && countNpoint > 0) {
			...
			if (countNpoint < 5) {
				...
				//if (max_temp < temp_average) max_temp = temp_average;
	...
	//read files of frequency and power
	...
	for (unsigned int i=0; i<name_filein.size(); i++) {
		...
		if (sel_file.Contains("-1") || sel_file.Contains("all")) {
			//GetData_rootFile(name_filein[i] , time_power, vec_freq, vec_power);
	...
	//outfilename += Form("Freq_Power_Tn_%s_%s_CenterFreq_211022.root", chan.Data(), sel_file.Data());
	outfilename += Form("Freq_Power_Tn_%s_%s_CenterFreq.root", chan.Data(), sel_file.Data());
	...
	for (unsigned int it = 0 ; it < vec_4step_time.size(); it++) {
		...
		for (int ij = 0; ij < vec_4step_time[it].size(); ij++) {
		...
		double time_ch7_e = vec_4step_time[it][NTimes-1];
		...
		for (unsigned int ip = 0; ip < vec_time_power.size(); ip++) {
			...
			if ( total_time >= (time_ch7_s -20) && total_time <= (time_ch7_e + 0) ) {
				...
				if (vec_run[it]==2 && vec_step[it]==1){
					cout << vec_str_time_power[ip] << endl;
					for (int ij = 0; ij < vec_vec_power[ip].size(); ij++) {
						cout << vec_vec_power[ip][ij] << endl;
					}
				}
