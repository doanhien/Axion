...
#include "/home/hien/work/axion/analysis/Code_Ana/Baseline_study/interface/weight.h"
...
void Combine_spectrum(TString strDirInput, int startFile = 1, int endFile = 16, TString cat = "Noise_meanP") {
  ...
  while (TSystemFile* ifile = (TSystemFile*)iterFile()) {
    ...
    //cout << arr_name->GetEntries() << endl;

    
    TString str_file_index = ((TObjString*) arr_name->At(3)) -> String();
    int file_index = str_file_index.Atoi();

    //cout << file_index << endl;

    if (file_index == 172 || file_index == 101) continue;
    if (nameFile.Contains("Kbin")) continue;
  ...
  printf("min_RFfreq: %.6f \n", min_RFfreq);
  printf("max_RFfreq: %.6f \n",  max_RFfreq );;
  iterFile . Reset();
  ...
  while (TSystemFile* ifile = (TSystemFile*)iterFile()) {
    ...
    //cout << "number of freqency bins: " << vec_freq.size() << " and power bin: " << vec_power.size() << endl;
  ...
  //vec_RF_freq . push_back(max_RFfreq);
