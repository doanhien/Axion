...
void Make_GrandSpectrum_v2 (TString indir, TString infileName, int Kbin = 5, int Lq_weight = 1, bool Mis_Align = true) {
	//Lq_weight = 1 : uniform shape
	//Lq_weight = 2 : Maxwellian shape
	//Lq_weight = 3 : Gaussian shape
	...
	if (Lq_weight > 1) {
		...
		for (int ip = 0; ip < vec_freq.size()-Kbin; ip++) {
			...
			for (int iq=0; iq<Kbin; iq++) {
				...
				if (!Mis_Align) {
					if (Lq_weight==2) Lq = Lq_calculator(start_freq, end_freq, 0., binwidth, iq);
					if (Lq_weight==3) Lq = Lq_calculator(start_freq, end_freq, 0., binwidth, iq, fwhm, mean_freq);
					vec_mean_Lq . push_back(Lq);
				}

				else {
					while (delta_nu < (1. - z)*binwidth ) {

						if (Lq_weight==2) Lq = Lq_calculator(start_freq, end_freq, delta_nu, binwidth, iq); //Maxwellian shape
						if (Lq_weight==3) Lq = Lq_calculator(start_freq, end_freq, delta_nu, binwidth, iq, fwhm, mean_freq); //Gaussian
		...
		for (int ip = 0; ip < vec_freq.size()-Kbin; ip++) {
			...
			for (int ij = ip; ij < ip+Kbin; ij++) {
				...
				if (Lq_weight > 1) sigma_rebin = vec_R_rebin[ij]/vec_mean_Lq[ij+(Kbin-1)*ip];
		...
		if (Lq_weight == 1) outfileName += "_UniformShape";
		if (Lq_weight == 2) outfileName += "_MaxwellianShape";
		if (Lq_weight == 3) outfileName += "_GaussianShape";
		if (Mis_Align)      outfileName += "_MisAlign";
		else                outfileName += "_NoMisAlign";
		
		outfileName += "_test.root";
