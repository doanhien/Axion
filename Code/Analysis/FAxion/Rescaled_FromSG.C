...
void Rescaled_FromSG(TString indir, int istep = 1, TString cat = "NoSignal", int order = 4, int window = 201) {


	//TString indir    = ("/home/hien/work/axion/analysis/Code_Ana/StrongFaxion_Study/output/SG_Filter/");

	//TString inFileName = Form("Bkg_step%04d_%s.root", istep, cat.Data());
	TString inFileName = Form("SGFilter_NPar_%d_Window_%d_%s_step%04d.root", order, window, cat.Data(), istep);
	...
	*/
	...
	for (int ie = 0; ie < intree->GetEntries(); ie++) {
		...
		if (abs(delta_f) < 1.E-6) printf("freq: %.7f noise: %.4f \n", freq, noise);
	...
	OutFileName += Form("Rescaled_Spectrum_Step_%04d_%s_newC.root", istep, cat.Data());
