...
void Rebin_Spectrum (TString infileName, vector<double> &vec_D_rebin, vector<double> &vec_R_rebin, int Cbin, int Kbin) {
  ...
  for (int ie = 0; ie < nentries; ie++ ) {
    ...
    if (ncount == Cbin) {
      vec_D_rebin . push_back(sum_D_ck);
      vec_R_rebin . push_back(sum_R_ck);
      ncount = 0;
      sum_D_ck = 0;
      sum_R_ck = 0;
    }
...
//void Make_GrandSpectrum (TString indir, TString infileName, TString run = "CD099", int Kbin = 5) {
void Make_GrandSpectrum (TString indir, TString infileName, int Kbin = 5) {
  ...
  for (int ip = 0; ip < vec_freq.size()-Kbin; ip++) {
	...
	//cout << "delta nu: " << delta_nu << endl;
	////double Lq = Lq_calculator(start_freq, end_freq, delta_nu);
  ...
  for (int ip = 0; ip < vec_freq.size()-Kbin; ip++) {
    ...
    for (int ij = ip; ij < ip+Kbin; ij++) {
      ...
      D_grand += vec_D_rebin[ij] * vec_mean_Lq[ij+(Kbin-1)*ip];
      R_grand += vec_R_rebin[ij] * pow(vec_mean_Lq[ij+(Kbin-1)*ip],2); //Haystac's formula
      R_grand_square += vec_R_rebin[ij] * vec_mean_Lq[ij+(Kbin-1)*ip];
      
      //D_grand += vec_D_rebin[ij];
      //R_grand += vec_R_rebin[ij];

      if (abs(vec_freq[ip] - 4.708972) < 10.E-6)  {
	printf("   --   = %.6f   %.6f %.4f  %.4f  %.5f  %.4f\n",
	       vec_freq[ip], vec_freq[ij], vec_mean_Lq[ij-ip], vec_D_rebin[ij], sqrt(vec_R_rebin[ij]), vec_D_rebin[ij]/vec_R_rebin[ij]);
	
	IsPrint = true;
      }
      
      //printf("   --   = %.6f   %.6f    %.6f  \n", vec_mean_Lq[ij+(Kbin-1)*ip], vec_D_rebin[ij], vec_R_rebin[ij]);
      
    }

    
    grand_power_sigma = 1./sqrt(R_grand);
    grand_power = D_grand * pow(grand_power_sigma,2);
    grand_freq  = vec_freq[ip];

    if (IsPrint) {
      printf("grand power: %.3f  grand sigma: %.3f \n", grand_power, grand_power_sigma);
      printf("\n");
    }
    ...
    //double snr = abs(grand_power/grand_power_sigma);
    //double G = (snr==0.0) ? 0.0 : sqrt(RT/snr);
